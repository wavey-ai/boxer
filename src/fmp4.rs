use crate::flac::parse_flac_header;
use access_unit::aac::extract_aac_data;
use access_unit::flac::{extract_flac_frame, parse_flac_frame_samples};
...
pub fn box_fmp4(
    ...
    if avcc.is_some() {
        ...
        let mut traf = TrackFragmentBox::new(TrackType::Video);
    ...
    match audio_type {
        AudioType::FLAC => {
            let mut flac_config: Option<FlacStreamConfiguration> = None;
            let mut flac_metadata: Vec<FlacMetadataBlock> = Vec::new();
            for (i, a) in audio_units.iter().enumerate() {
                if i == 0 {
                    let (config, metadata) = parse_flac_header(&a.data);
                    flac_config = Some(config);
                    flac_metadata = metadata;
                }

                if let Some(config) = &flac_config {
                    let raw_audio = extract_flac_frame(&a.data);
                    match parse_flac_frame_samples(raw_audio) {
                        Ok(frame_samples) => {
                            let frame_duration =
                                (frame_samples as f64 / config.sample_rate as f64 * 1000.0) as u32;
                        ...
                        Err(e) => {
                            eprintln!("Error parsing FLAC frame: {}", e);
                            // Handle error (skip frame, use default duration, etc.)
                        }
                    }
                }
            }
            if let Some(config) = flac_config {
                ...
                audio_track.mdia_box.mdhd_box.timescale = config.sample_rate;
                audio_track.mdia_box.mdhd_box.duration = 0;

                let flac_sample_entry = FlacSampleEntry {
                    dfla_box: config,
                    flac_box: flac_metadata,
                };
                audio_track
                    .mdia_box
                    .minf_box
        ...
        AudioType::AAC => {
            ...
            if !audio_units.is_empty() {
                let mut traf = TrackFragmentBox::new(TrackType::Audio);
                traf.tfhd_box.default_sample_duration = None;
                traf.trun_box.data_offset = Some(0);
                traf.trun_box.samples = audio_samples;
                traf.tfdt_box.base_media_decode_time =
